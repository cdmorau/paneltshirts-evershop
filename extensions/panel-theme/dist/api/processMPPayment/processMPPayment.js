...
export default (async (request, response)=>{
    ...
    if (!formData || !order_id) {
        return response.status(400).json({
            error: {
                message: 'formData y order_id son requeridos'
            }
        });
    }
    try {
        ...
        if (!order) return response.status(404).json({
            error: {
        ...
        await queryDB(`UPDATE "order" SET payment_status = $1, updated_at = NOW() WHERE uuid = $2`, [
            newStatus,
            order_id
        ]);
